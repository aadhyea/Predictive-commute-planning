...
def render_app():
    ...
    with st.sidebar:
        ...
        if _sidebar_user:
            saved = get_client().get_saved_commutes(_sidebar_user.id) or []
            if saved:
                with st.expander("🔖 Saved Commutes", expanded=False):
                    for c in saved:
                        ...
                        with col_btn:
                            if st.button(display_name, key=f"saved_{c['id']}", use_container_width=True):
                                _quick_fill(c["origin"], c["destination"])
                        with col_del:
                            if st.button("✕", key=f"del_{c['id']}", help="Remove saved commute", type="tertiary"):
